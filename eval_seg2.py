...
# =========================================================
# 🔥 新增：NPR 专家网络类定义 (从 eval_cls 完美移植)
# =========================================================
...
def parse_args():
    parser = argparse.ArgumentParser(description="LISA Mask Evaluation (Dynamic Hints)")
    ...
    parser.add_argument("--version", default="./checkpoints_stage2/wo_feather_half_hint_epoch2/merged_final")
...
def main():
    ...
    batch_size = 32
    for i in tqdm(range(0, len(fake_data), batch_size), desc="Injecting Expert Hints"):
        ...
        if img_tensors:
            ...
            for valid_idx, pred in zip(valid_indices, preds):
                if pred == 1:
                    ...
                    # 如果专家网络预测错误（没认出是假图），这里会注入真实的、错误的判断（测试主模型抗忽悠能力！）
    ...
    for item in pbar:
        ...
        # 🔥 核心修改：移除硬编码，使用专家网络动态生成的提示词
        dynamic_expert_hint = item.get("expert_hint", "")
        prompt = base_prompt + "\n" + dynamic_expert_hint
