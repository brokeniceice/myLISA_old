...
def parse_args():
    ...
    parser.add_argument("--test_json", type=str, default="./datasets/LOKI/true_or_false.json")
...
def resolve_dataset_root(args):
    ...
    return os.path.dirname(os.path.abspath(args.test_json))
...
def resolve_image_path(item, dataset_root):
    ...
    if not raw_path:
        ...
        raw_path = images[0] if images else ""
    ...
    candidates = [
        ...
        os.path.join(dataset_root, raw_path.lstrip("./")),
    ]
...
def extract_ground_truth_class(item):
    question_type = str(item.get("question_type", "")).lower()

    if "image_tf_image_fake_" in question_type:
        return 1
    if "image_tf_image_real_" in question_type:
        return 0
...
def deduplicate_by_image(test_data):
    unique_items = []
    seen_images = set()

    for item in test_data:
        image_key = item.get("image_path") or item.get("image") or ""
        if not image_key:
            images = item.get("images", [])
            image_key = images[0] if images else ""
...
def main():
    ...
    test_data = deduplicate_by_image(test_data)
    print(f"✅ 已按图片去重: {original_count} -> {len(test_data)} 条")
    ...
    if args.disable_expert_hint:
        ...
        for i in tqdm(range(0, len(test_data), batch_size), desc="Injecting Expert Hints"):
            ...
            for idx, item in enumerate(batch_items):
                img_path = resolve_image_path(item, dataset_root)
    ...
    for idx, item in enumerate(pbar):
        # --- 获取 GT 标签：由 question_type + answer 组合解析 ---
        gt_class = extract_ground_truth_class(item)
