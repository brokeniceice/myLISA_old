...
def parse_args():
    ...
    parser.add_argument("--data_root", type=str, default="./datasets/Fakebench")
...
def load_fakebench_data(dataset_root):
    test_data = []
    fake_dir = os.path.join(dataset_root, "fake_images")
    real_dir = os.path.join(dataset_root, "real_images")

    for image_path in list_image_files(fake_dir):
        test_data.append({
            "image_path": os.path.relpath(image_path, dataset_root),
            "label": 1,
        })

    for image_path in list_image_files(real_dir):
        test_data.append({
            "image_path": os.path.relpath(image_path, dataset_root),
            "label": 0,
        })
...
def main():
    ...
    print(f"📂 读取测试集目录: {dataset_root}")
    test_data = load_fakebench_data(dataset_root)
