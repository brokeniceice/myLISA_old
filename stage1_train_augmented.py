...
swanlab.init(
    ...
    config={
        ...
        "expansion": "7x",
        "split": "8:1:1",
        "note": "Strictly aligned with evaluate_robustness.py pipeline"
    }
)
...
val_dataset = MapDataset(val_subset) # 默认使用 Clean 原图对应的 base_val_transform
...
# ----------------------------
print("=" * 60)
print(f"📊 数据集切分 (8:1:1) 完毕 | CV2底座对齐:")
print(f"   - 基础训练集 (Train)   : {train_size} 张 -> (扩容后: {len(train_dataset)} 张)")
print(f"   - 验证集 (Val)         : {val_size} 张")
