...
class StableMultiHeadCrossAttention(nn.Module):
...
class LlavaMetaModel:
    def __init__(self, config):
        ...
        #引入交叉注意力融合
        self.npr_cross_attn = StableMultiHeadCrossAttention(dim=config.hidden_size,num_heads=8)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        npr_ckpt = getattr(model_args, "npr_pretrained_path", None)
        if npr_ckpt is not None:
        ...
        if not hasattr(self, "npr_projector"):
            self.npr_projector = nn.Sequential(
            nn.Linear(self.mm_hidden_size, self.config.hidden_size),
            nn.GELU(),
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        # 1. CLIP 提取宏观语义特征 [Batch, 256, 4096]
        image_features = self.get_model().get_vision_tower()(images)
        image_features = self.get_model().mm_projector(image_features)

        # ==========================================
        # 🟢 动态消融开关 (Ablation Switch)
        # ==========================================
        images_npr = getattr(self.get_model(), "current_images_npr", None)
        
        # 如果训练/推理脚本里没有传 NPR 图，直接退化为单分支！
        if images_npr is None:
        ...
        npr_features = npr_features.permute(0, 2, 1)   
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            if (
                ...
                attention_mask = torch.ones(
                    (
                        attention_mask.shape[0] if attention_mask is not None else input_ids.shape[0], 
                        past_key_values.get_seq_length() + 1 if hasattr(past_key_values, "get_seq_length") else past_key_values[-1][-1].shape[-2] + 1
                    ),
